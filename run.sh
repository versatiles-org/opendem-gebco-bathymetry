...
if [ ! -f "./tmp/bathymetry.mbtiles" ]; then
	...
	TILEJSON=$(sqlite3 "./tmp/bathymetry-high.mbtiles" "SELECT value FROM metadata WHERE name = 'json';" | jq -c '.vector_layers[0] |= . + { "attribution": "Derived product from the <a href=\"https://www.gebco.net/data_and_products/historical_data_sets/#gebco_2019\">GEBCO 2019 Grid</a>, made with <a href=\"https://www.naturalearthdata.com/\">NaturalEarth</a> by <a href=\"https://opendem.info\">OpenDEM</a>", "name": "OpenDEM GEBCO Bathymetry", "description": "Bathymetry depth vector layers, created by OpenDEM <https://opendem.info/> using Data from GEBCO 2021 Grid <https://www.gebco.net/data_and_products/gridded_bathymetry_data/> and Natural Earth <https://www.naturalearthdata.com/>" }');

	sqlite3 "./tmp/bathymetry.mbtiles" <<EOF
INSERT OR REPLACE INTO metadata (name, value) VALUES ('attribution', 'Derived product from the <a href="https://www.gebco.net/data_and_products/historical_data_sets/#gebco_2019">GEBCO 2019 Grid</a>, made with <a href="https://www.naturalearthdata.com/">NaturalEarth</a> by <a href="https://opendem.info">OpenDEM</a>');
